...
<form action="index.php?option=com_minitheatrecm&view=ctypes" method="post" id="adminForm" name="adminForm" class="clearfix">
	...
	<?php endif; ?>
		...
		<table class="table table-striped table-hover">
			<thead>
				<tr>
					...
					<th width="67%" class="nowrap">
						<?php echo JHtml::_('searchtools.sort', 'COM_MINITHEATRECM_MAINCOLUMN_CTYPE', 'a.name', $listDirn, $listOrder) ;?>
					</th>
			...
			<tbody>
				...
				<?php foreach ($this->items as $i => $row) : $link = JRoute::_('index.php?option=com_minitheatrecm&task=ctype.edit&id=' . $row->id); ?>
				<tr>
					<td class="center">
						<?php echo JHtml::_('grid.id', $i, $row->id); ?>
					</td>
					<td class="nowrap center">
						<?php echo NeonHtmlManager::renderTaskButtons( $row->state, $i, 'ctypes.', true, $row->publish_up, $row->publish_down );?>
					</td>
					<td>
						<div class="pull-left">
							<div>
								<?php echo NeonHtmlManager::renderMain($this->escape($row->name), $link, 'COM_MINITHEATRECM_TOOLTIP_EDIT_CTYPE')?>
							</div>
